...
func (ss *signalService) UpdateStrategy(
	...
	wl.Debugf("Account Leverage: %d\n", strategy.AccountLeverage)
	wl.Debugf("Account Leverage Is Zero: %t\n", strategy.AccountLeverage.ValueOrZero())
	wl.Debugf("Existing Account Leverage: %d\n", existing.AccountLeverage)
	if !strategy.AccountLeverage.IsZero() && existing.AccountLeverage != strategy.AccountLeverage {
		existing.AccountLeverage = strategy.AccountLeverage
		isDifferent = true
	}

	if !strategy.FixedTradeAmount.IsZero() && existing.FixedTradeAmount != strategy.FixedTradeAmount {
