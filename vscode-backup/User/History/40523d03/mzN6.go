...
func (sd *strategyDAL) GetStrategies(
	...
	query := "SELECT id, enabled, user_id, signal_source_id, type, name, exchange, margin, leverage, trade_strategy, fixed_trade_amount, currency_pair, created_at, updated_at, deleted_at FROM mvp_strategy"
	err := sd.db.Select(&rows, query)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
