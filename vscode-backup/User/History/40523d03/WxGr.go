...
func (sd *strategyDAL) GetActiveStrategiesForSignalSourceAndUser(
	...
	query := "SELECT id, enabled, user_id, signal_source_id, type, name, exchange, margin, leverage, trade_strategy, fixed_trade_amount, currency_pair FROM mvp_strategy WHERE enabled = TRUE AND signal_source_id = $1 AND user_id = $2 ORDER BY name DESC"
...
func (sd *strategyDAL) GetActiveStrategies(
	...
	query := "SELECT id, enabled, user_id, signal_source_id, type, name, exchange, margin, leverage, trade_strategy, fixed_trade_amount, currency_pair FROM mvp_strategy WHERE enabled = TRUE ORDER BY name DESC"
...
func (sd *strategyDAL) GetStrategies(
	...
	query := "SELECT id, enabled, user_id, signal_source_id, type, name, exchange, margin, leverage, trade_strategy, fixed_trade_amount, currency_pair, created_at, updated_at, deleted_at FROM mvp_strategy ORDER BY name DESC"
	err := sd.db.Select(&rows, query)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
