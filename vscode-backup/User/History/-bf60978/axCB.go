...
func (ss *signalService) tradeSpotStrategy(
	...
	if desiredSide.IsEquivalent(coinroutesapi.Short) {
		// handle short/sell same as neutral
		wl.Debug("desired side is short, setting to neutral")
	...
	if position == nil {
		...
		wl.Debugf("found neutral state. desired side: %s", desiredSide)

		if desiredSide.IsEquivalent(coinroutesapi.Neut) {
			// log signal and no-op
			wl.Debug("desired side is neutral, no-op")
	...
	case coinroutesapi.Long, coinroutesapi.Buy:
		...
		// if desiredSide is long/buy, we trade position (go long)
		...
		return nil
	case coinroutesapi.Short, coinroutesapi.Neut, coinroutesapi.Sell:
		// if desiredSide is short/sell, we trade neutral
		// if desiredSide is neutral, we trade neutral
		wl.Debug("desired side is neutral")
		desiredSide = coinroutesapi.Neut
		// neutral means buying usdt. set currencyPair accordingly
		var neutralPair coinroutesapi.CurrencyPairType

		switch currencyPair {
		case coinroutesapi.USDBTCPerpetual, coinroutesapi.USDTBTCPerpetual:
			neutralPair = coinroutesapi.USDTBTCPerpetual
		case coinroutesapi.USDETHPerpetual, coinroutesapi.USDTETHPerpetual:
			neutralPair = coinroutesapi.USDTETHPerpetual
		}

		err := ss.tradeNeutral(ctx, wl, chain, strategy, signal, position, neutralPair)
		if err != nil {
