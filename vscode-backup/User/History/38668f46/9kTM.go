...
func HTMLTable(ctx context.Context, wl wlog.Logger, w http.ResponseWriter, data []interface{}) {
	...
	for i := 0; i < numFields; i++ {
		...
		table += "<th style=\"border: 1px solid black; padding: 8px;\">" + field.Name + "</th>"
	}
	table += "</tr>"
	...
	for _, item := range data {
		...
		for i := 0; i < values.NumField(); i++ {
			...
			table += "<td style=\"border: 1px solid black; padding: 8px;\">" + fmt.Sprintf("%v", value.Interface()) + "</td>"
		}
		table += "</tr>"
	}
	...
	// Add CSS styles for printer-friendly layout
	printerFriendlyCSS := `
	<style>
		table {
	...
	// Write the HTML table with printer-friendly CSS to the response
	if _, err := w.Write([]byte(printerFriendlyCSS + table)); err != nil {
		wl.Error(err)
	}
}
