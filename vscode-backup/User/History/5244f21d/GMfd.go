...
const (
	LongShort    StrategyType = "long_short"
	LongNeutral  StrategyType = "long_neutral"
	ShortNeutral StrategyType = "short_neutral"
...
const (
	...
	OKX     ExchangeType = "okex"
	Deribit ExchangeType = "deribit"
	DYDX    ExchangeType = "dydx"
	Vertex  ExchangeType = "vertex_arb"
)

// validate values
func (e ExchangeType) Validate() error {
	switch e {
	case Binance, FTX, BYBIT, Kraken, OKX, Deribit, DYDX, Vertex:
		return nil
	default:
		return fmt.Errorf("invalid exchange type: %s", e)
	}
}
...
const (
	...
	CoinD MarginType = "coindemoninated"
	Spot  MarginType = "spot"
)

// validate margin type
func (m MarginType) Validate() error {
	switch m {
	case USDTM, USDM, CoinM, Spot, CoinD:
		return nil
	default:
		return fmt.Errorf("invalid margin type: %s", m)
	}
}
...
type Strategy struct {
	...
	AccountLeverage  null.Int          `db:"account_leverage" json:"account_leverage"`
...
func (orr OrderReportRecord) GetHeaders() []string {
	...
	for i := 0; i < t.Type().NumField(); i++ {
		h = append(h, t.Type().Field(i).Name)
	}
	return h
}

func (orr OrderReportRecord) GetValues() []string {
	t := reflect.ValueOf(orr)
	var v []string

	for i := 0; i < t.NumField(); i++ {
		v = append(v, t.Field(i).String())
	}
	return v
}
