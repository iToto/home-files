...
func (c *Client) GetSignalFromIPV2(
	...
	if signalSource.TLS {
		...
		url = fmt.Sprintf("http://%s", signalSource.IP)
	}
	req, err := client.NewJSONRequest(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, fmt.Errorf("error building signal request: %w", err)
	...
	lastCheckedTime, err := time.Parse(signalSource.TimeFormat, res.LastChecked)
	if err != nil {
		wl.Debugf("invalid last_checked_time: %s", res.LastChecked)
	}
	lastTradeSignalTime, err := time.Parse(signalSource.TimeFormat, res.LastTradeSignalTime)
	if err != nil {
	...
	signal := &SignalResponseV2{
		...
		LastTradeSignal:     SignalType(strings.ToLower(res.LastTradeSignal)),
