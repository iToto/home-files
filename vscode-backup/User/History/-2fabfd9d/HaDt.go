...
type DAL interface {
	...
	// passing false for returnCoin will return the balance in USD, passing true will
	// return the balance in the coin's native currency
	...
	for _, v := range *balances {
		...
		if strings.ToLower(v.Currency) == string(chain) &&
			...
			if returnCoin {
				...
				amt, err = strconv.ParseFloat(v.USDValue, 32)
			}
			if err != nil {
				return amt, err
			}
		...
		// handle dydx also using usdc
		if v.Exchange == string(entities.DYDX) &&
			strategy.Exchange == entities.DYDX &&
			strings.ToLower(v.Currency) == string(coinroutesapi.USDC) {
			if returnCoin {
				amt, err = strconv.ParseFloat(v.Amount, 32)
			} else {
				amt, err = strconv.ParseFloat(v.USDValue, 32)
...
func (es *exchangeService) GetPositionForStrategy(
	...
	if strategy.Margin == entities.Spot {
		...
		for _, b := range *balances {
			if b.Currency == currency {
				...
				usdValue, err := strconv.ParseFloat(b.USDValue, 64)
				if err != nil {
					wl.Infof("error found when parsing USD price to float from balance, err: %s", err)
					usdValue = 0
				}
	...
	for _, v := range *positions {
		...
		if v.CurrencyPair == string(currencyPair) {
			...
			usdValue, err := strconv.ParseFloat(v.UsdValue, 64)
			if err != nil {
				wl.Infof("error found when parsing USD price to float, err: %s", err)
				usdValue = 0
			}
