...
type DAL interface {
	...
	// GetPositionForStrategyV2 will get the the current position for a strategy
	// it will not alter the values returned from the exchange as
	// GetPositionForStrategy does
	GetPositionForStrategyV2(
		ctx context.Context,
		wl wlog.Logger,
		strategy *entities.Strategy,
		currencyPair coinroutesapi.CurrencyPairType,
...
func (es *exchangeService) GetPositionForStrategyV2(
	ctx context.Context,
	wl wlog.Logger,
	strategy *entities.Strategy,
	currencyPair coinroutesapi.CurrencyPairType,
