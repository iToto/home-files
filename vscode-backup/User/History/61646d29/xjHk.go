...
func GetStrategies(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if enabled, err := strconv.ParseBool(vars.Get("enabled")); err == nil {

			if err != nil {
				render.BadRequest(ctx, wl, w, fmt.Errorf("invalid enabled value"))
				return
			}
			wl.Debugf("enabled filter: %v", enabledBool)
		}

		strats, err := signalService.GetStrategies(ctx, wl)
		if err != nil {
			render.InternalError(ctx, wl, w, err)
			return
		}
