...
func WSConnection(path string, wl wlog.Logger) (*websocket.Conn, error) {
	...
	if path == "" {
		return nil, fmt.Errorf("invalid path for websocket connection: ", path)
	}
...
func main() {
	...
	// BTC Strategies
	btcLongShortStrategy := os.Getenv("BTC_LONG_SHORT")
	if btcLongShortStrategy == "" {
		log.Fatal("$BTC_LONG_SHORT environment variable must be set")
	}
	btcLongNeutralStrategy := os.Getenv("BTC_LONG_NEUTRAL")
	if btcLongShortStrategy == "" {
		log.Fatal("$BTC_LONG_NEUTRAL environment variable must be set")
	}
	btcFTXLongShortStrategy := os.Getenv("BTC_FTX_LONG_SHORT")
	if btcFTXLongShortStrategy == "" {
		log.Fatal("$BTC_FTX_LONG_SHORT environment variable must be set")
	}

	// ETH Strategies
	ethLongShortStrategy := os.Getenv("ETH_LONG_SHORT")
	if btcLongShortStrategy == "" {
		log.Fatal("$ETH_LONG_SHORT environment variable must be set")
	}
	ethLongNeutralStrategy := os.Getenv("ETH_LONG_NEUTRAL")
	if btcLongShortStrategy == "" {
		log.Fatal("$ETH_LONG_NEUTRAL environment variable must be set")
	}
	ethUSDTStrategy := os.Getenv("ETH_USDT")
	if ethUSDTStrategy == "" {
		log.Fatal("$ETH_USDT environment variable must be set")
	}
	...
	ethLS := entities.Strategy{
		Type:     entities.LongShort,
		Name:     ethLongShortStrategy,
		Exchange: entities.Binance,
	}
	...
	ethChain := entities.Chain{
		Type:       entities.ETH,
		Strategies: []entities.Strategy{ethLN, ethLS, ethUSDT},
	}
	// BTC
	...
	btcChain := entities.Chain{
		Type:       entities.BTC,
		Strategies: []entities.Strategy{btcLN, btcLS, btcFTXLS},
	}
	...
	btcTicker.Run(context.Background(), wl)
	ethTicker.Run(context.Background(), wl)
	...
	router.HandleFunc(
		...
		handler.GetBTCSignal(
			wl,
			chains[entities.BTC].Strategies,
			signalService)).Methods(http.MethodGet, http.MethodOptions)
	router.HandleFunc(
		"/b",
