...
import (
	...
	"flag"
	"strconv"
	"yield-mvp/cmd/yield-mvp/internal/tickr"
...
func main() {
	...
	btcPriceConsumer.Start(btcWS, wl)
	ethPriceConsumer.Start(ethWS, wl)
	...
	if btcLongShortStrategy == "" {
		log.Fatal("$LONG_SHORT environment variable must be set")
	}
	btcLongNeutralStrategy := os.Getenv("BTC_LONG_NEUTRAL")
	if btcLongShortStrategy == "" {
		log.Fatal("$LONG_NEUTRAL environment variable must be set")
	}

	btcStrategies := [...]string{btcLongNeutralStrategy, btcLongShortStrategy}
	...
	ethStrategies := [...]string{ethLongNeutralStrategy, ethLongShortStrategy}
	...
	ethChain := entities.Chain{
		Type:       entities.ETH,
		Strategies: []entities.Strategy{ethLN, ethLS},
	}
	// BTC
	...
	btcChain := entities.Chain{
		Type:       entities.BTC,
		Strategies: []entities.Strategy{btcLN, btcLS},
	}
	chains := []entities.Chain{btcChain, ethChain}
