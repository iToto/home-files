...
import (
	...
	legacy_acquiring_restapi "github.com/wingocard/braavos/internal/legacy/handler/restapi/acquiring"
	...
	"github.com/wingocard/braavos/internal/rest/handler/kychdl"
...
func New(wl wlog.Logger, bs *backingservice.BackingServices) (*mux.Router, error) {
	...
	v1Public.Handle("/teen", userhdl.CreateTeen(
		wl,
		bs.UserService,
	)).Methods(http.MethodPost, http.MethodOptions)
	...
	v1.HandleFunc("/parent/kyc",
		kychdl.RunParentKYC(wl, bs.KYCService),
	...
	v2.HandleFunc("/funding_account",
		legacy_acquiring_restapi.HandleCreateFundingAccountV2(
			wl,
			bs.Legacy.AcquiringService,
...
func addLegacyV1Routes(
	...
	v1ActiveUsers.HandleFunc("/funding_account",
		legacy_acquiring_restapi.HandleGetFundingAccounts(wl, bs.Legacy.AcquiringService),
	).Methods(http.MethodGet)
	v1ActiveUsers.HandleFunc("/funding_account_encryption_key",
		legacy_acquiring_restapi.HandleGetEncryptionKey(wl, bs.Legacy.AcquiringService),
	).Methods(http.MethodGet)
	v1ActiveUsers.HandleFunc("/fund_bank_account",
		legacy_acquiring_restapi.HandleFundAccount(
			wl,
			bs.Legacy.AcquiringService,
			bs.Legacy.UserService,
			bs.Legacy.IssuingService,
