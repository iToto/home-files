...
func (ss *signalService) queryLastSignalForStrategy(
	...
	query := "SELECT id, chain, signal, strategy, trade_time, created_at, updated_at, deleted_at FROM mvp_signal_log WHERE strategy = $1 ORDER BY created_at DESC LIMIT 1"
	err := ss.db.Get(&lastSignal, query, strategy)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			wl.Info("no signal history found")
...
func (ss *signalService) getCurrencyPairForStrategy(
	ctx context.Context,
	wl wlog.Logger,
	chain entities.ChainType,
	strategy coinroutesapi.SupportedStrategy,
) (*coinroutesapi.CurrencyPairType, error) {
	var currencyPair coinroutesapi.CurrencyPairType

	switch chain {
	case entities.BTC:
		switch strategy {
		case coinroutesapi.BTCLongNeutral:
			currencyPair = coinroutesapi.BTCInversePerpetual
		case coinroutesapi.BTCLongShort:
			currencyPair = coinroutesapi.BTCInversePerpetual
		case coinroutesapi.BTCFTXLongShort:
			currencyPair = coinroutesapi.USDBTCPerpetual
		default:
			return nil, fmt.Errorf("invalid strategy found")
		}
	case entities.ETH:
		switch strategy {
		case coinroutesapi.ETHLongNeutral:
			currencyPair = coinroutesapi.ETHInversePerpetual
		case coinroutesapi.ETHLongShort:
			currencyPair = coinroutesapi.ETHInversePerpetual
		case coinroutesapi.ETHUSDT:
	...
	default:
		return nil, fmt.Errorf("invalid chain found: %s", chain)
	}

	wl.Debugf("currency_pair set to %s", currencyPair)
