...
func (ss *signalService) checkForSignal(
	...
	lastSignal, err := ss.queryLastSignalForChain(ctx, wl, sig.Chain)
	if err != nil {
		...
		// log signal to BQ
		err = ss.sl.Log(ctx, wl, delta, sig, signal.ID) // FIXME: Should be passing internal entities
		if err != nil {
...
func (ss *signalService) queryLastSignalForStrategy(
	...
	query := "SELECT * FROM mvp_signal_log WHERE strategy = $1 ORDER BY created_at DESC LIMIT 1"
	err := ss.db.Get(&lastSignal, query, strategy)
	if err != nil {
		wl.Debugf("error querying signal for strategy %s with err: %s", strategy, err)
		return nil, err
	}
...
func (ss *signalService) insertLatestSignalTradedForChain(
	ctx context.Context,
	wl wlog.Logger,
	chain entities.ChainType,
	signal *entities.Signal,
...
func (ss *signalService) parseSignalAndCreateTradePayload(
	...
	if cur == coinroutesapi.BTC {
		...
		if str == coinroutesapi.ETHUSDT { // check if we are trading USDT
	...
	if sig.Signal == entities.Buy || sig.Signal == entities.Long {
		...
		// if we are using USDT, trade in funding currency
		if str == coinroutesapi.ETHUSDT {
			payload.UseFundingCurrency = true
		}
...
func (ss *signalService) calculateOrderQuantity(
	...
	wl wlog.Logger,
	chain entities.ChainType,
	strategy coinroutesapi.StrategyType,
	payload *coinroutesapi.ClientOrderCreateRequest,
) (float64, error) {
	if strategy == coinroutesapi.ETHUSDT {
		...
		// If Neutral(no contracts): Check balance and make order for 1x fixed amount
		if position == nil {
			...
			if balance >= usdtTradeAmount {
				wl.Debugf("balance is greater than trade amt %f >= %f", balance, usdtTradeAmount)
				return usdtTradeAmount, nil
			}

			wl.Debugf("balance not greater than trade amt %f < %f", balance, usdtTradeAmount)
			return balance, nil
		}

		// If Long/Short(contracts): Confirm contract side is opposite,
		// make order for 2x fixed amount
		if position.Side == strings.ToLower(string(payload.Side)) {
			return 0.0, fmt.Errorf(
				"position is already in desired state: %s : %s",
				position.Side,
				strings.ToLower(string(payload.Side)),
			)
		}

		wl.Debugf("changing position, returning 2x trade amount: %f", 2*usdtTradeAmount)
		return 2 * usdtTradeAmount, nil
		...
		case coinroutesapi.Sell:
			...
			if strategy == coinroutesapi.BTCLongShort || strategy == coinroutesapi.ETHLongShort {
