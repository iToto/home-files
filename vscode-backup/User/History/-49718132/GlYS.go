...
import (
	...
	"github.com/iToto/yeild-mvp/wcontext"
...
func WithServiceRequest(ctx context.Context, l Logger, serviceName string) Logger {
	if requestID, ok := ctx.Value(wcontext.ContextKeyRequestIDHeader).(string); ok {
		l = l.WithStr("requestID", requestID)
	}
	if callerID, ok := ctx.Value(wcontext.ContextKeyCallerIDHeader).(string); ok {
		l = l.WithStr("callerID", callerID)
	}
	if userID, ok := ctx.Value(wcontext.ContextKeyUserID).(string); ok {
		l = l.WithStr(LogKeyUserID, userID)
	}
	if traceID, ok := ctx.Value(wcontext.ContextKeyTraceIDHeader).(string); ok {
