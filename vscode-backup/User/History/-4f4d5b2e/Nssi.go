	...
	if position == nil {
		...
		case entities.Compound:
			...
			if position.UnrealizedPnl == 0 {
				upnl := 0.0
				currentPrice := 0.0 // TODO: get this from market price

				// manually calculate if it isn't set
				if position.Side == string(coinroutesapi.Short) ||
					position.Side == string(coinroutesapi.Sell) {
					// -1 X (Current price / entry price -1)
					upnl = -1 * (currentPrice/position.EntryPrice - 1)

				} else {
					// (Current price / entry price -1)
					upnl = (currentPrice/position.EntryPrice - 1)
				}
