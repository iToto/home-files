...
func (ss *signalService) GetAndProcessSignal(
	...
	signals map[entities.ChainType]entities.SignalSource,
) error {
	...
	switch chain {
	case entities.BTC:
		signalResp, err = ss.sc.GetSignalFromIPV2(ctx, wl1, signals[entities.BTC])
		if err != nil {
			return ErrSignalClient
		}
	case entities.ETH:
		signalResp, err = ss.sc.GetSignalFromIPV2(ctx, wl1, signals[entities.ETH])
		if err != nil {
			return ErrSignalClient
		}
	default:
	...
	for _, currentStrategy := range strats {
		coinRoutesStrategyName := coinroutesapi.SupportedStrategy(currentStrategy.Name)
		...
		if err != nil {
			...
			if errors.Is(err, ErrNoSignalHistory) {
				...
				err = ss.insertLatestSignalTradedForStrategy(ctx, wl, chain, signal, &currentStrategy)
				if err != nil {
					wl.Error(err)
				}
				continue
		...
		*********************************/
		// TODO: This needs to be also checked in trade_usdt_strategy.go
		if signal.Signal == entities.Neutral {
			wl.Debug("found neutral signal")
			crStrategy := coinroutesapi.SupportedStrategy(currentStrategy.Name)
			cp := currentStrategy.CurrencyPair.String()
			cp, err := ss.getCurrencyPairForStrategy(ctx, wl, chain, crStrategy)
			if err != nil {
				wl.Error(fmt.Errorf("could not get currency_pair when going neutral: %w", err))
				continue
			}
			...
			if position == nil {
				...
				wl.Debug("no-op as no positions found so already in neutral state")
				continue
			}
			// trade neutral
			ss.tradeNeutral(ctx, wl, chain, crStrategy, signal, position, *cp)
		...
		if currentStrategy.Exchange == entities.FTX {
			err := ss.tradeUSDTStrategy(ctx, wl, chain, &currentStrategy, desiredSide, signal)
			if err != nil {
		...
		***********************/
		...
		// set amount
		amount, err := ss.calculateOrderQuantity(ctx, wl, chain, &currentStrategy, payload)
		if err != nil {
