...
func (ss *signalService) tradeCoinDStrategy(
	...
	if currentSide.IsEquivalent(entities.Neutral) && desiredSide == coinroutesapi.Buy {
		...
		tradeAmount = balance * chainMarketPrice
	} else if currentSide.IsEquivalent(entities.Long) && desiredSide == coinroutesapi.Neutral {
		// set tradeAmount for the $ value of the Quantity
		tradeAmount = position.Quantity * chainMarketPrice

	// absolute value
	tradeAmount = math.Abs(tradeAmount)

	wl.Debugf("coind-debug: tradeAmount before rounding: %f", tradeAmount)

	dollarAmount := tradeAmount * chainMarketPrice

	// debug dollar amount
	wl.Debugf("coind-debug: dollarAmount before rounding: %f", dollarAmount)
	...
	if chain == entities.BTC {
		if strategy.Exchange == entities.Deribit {
			wl.Debug("coind-debug: rounding to nearest 10 BTC for Deribit")
	...
	// debug dollar amount
	wl.Debugf("coind-debug: dollarAmount after rounding: %f", dollarAmount)

	tradeAmount = dollarAmount / chainMarketPrice

	// debug trade amount
	wl.Debugf("coind-debug: tradeAmount after rounding: %f", tradeAmount)
	...
	wl.Debugf("coind-debug: currentSide: %s", currentSide)
	wl.Debugf("coind-debug: currentContractExposure: %f", currentContractExposure)
	wl.Debugf("coind-debug: signalMultiplier: %d", signalMultiplier)
	wl.Debugf("coind-debug: desiredExposure: %f", desiredExposure)
