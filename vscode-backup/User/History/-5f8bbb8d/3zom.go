...
func (ss *signalService) tradeCoinMStrategyV2(
	...
	switch {
	case totalExposure <= 0.1 && totalExposure >= -0.1:
		currentSide = entities.Neutral
	case totalExposure > 0.1:
	...
	if strategy.TradeStrategy == entities.Compound {
		...
		if entities.SignalType(desiredSide) == entities.Neutral {
			targetExposure = 0
		} else {
			targetExposure = b * float64(strategy.Leverage)
		}
	...
	wl.Debugf("coinm-debug: b: %f", b)
	wl.Debugf("coinm-debug: q: %f", q)
	wl.Debugf("coinm-debug: leverage %d", strategy.Leverage)
	wl.Debugf("coinm-debug: TradeStrategy %s", strategy.TradeStrategy)
	wl.Debugf("coinm-debug: totalExposure: %f", totalExposure)
	...
	// override side based on the trade amount. Positive is Buy, negative is Sell
	if tradeAmount > 0 {
