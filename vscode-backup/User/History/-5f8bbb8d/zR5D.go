...
func (ss *signalService) tradeCoinMStrategyV2(
	...
	// get current side
	currentSide, err := ss.calculateCurrentSide(ctx, wl, position, positionPercent, strategy)
	if err != nil {
		return fmt.Errorf("unable to calculate current side: %w", err)
	}

	wl.Debugf("coin-debug: currentSide: %s", currentSide)
	wl.Debugf("coin-debug: positionPercent: %f", positionPercent)
	wl.Debugf("coin-debug: coinValue: %f", coinValue)
	wl.Debugf("coin-debug: multiplier: %d", multiplier)
	wl.Debugf("coin-debug: accountDollarValue: %f", accountDollarValue)
	...
	tradeAmount, err := ss.calculateTradeAmount(ctx, wl, desiredExposure, currentContractExposure, accountDollarValue, chainMarketPrice, strategy)
	if err != nil {
	...
	wl.Debugf("coin-debug: desiredSide: %s", desiredSide)
	...
	wl.Debugf("coin-debug: chainMarketPrice: %f", chainMarketPrice)
	wl.Debugf("coin-debug: currentSide: %s", currentSide)
	wl.Debugf("coin-debug: currentContractExposure: %f", currentContractExposure)
	wl.Debugf("coin-debug: signalMultiplier: %d", signalMultiplier)
	wl.Debugf("coin-debug: desiredExposure: %f", desiredExposure)
	wl.Debugf("coin-debug: tradeAmount: %f", tradeAmount)

	var orderPayload coinroutesapi.ClientOrderCreateRequest
	useFundingCurrency := false

	// override for coindemoninated
	if strategy.Margin == entities.CoinD {
		// override current side.
		if position == nil {
		...
		// if we are in desired state, no-op
		if currentSide.IsEquivalent(entities.SignalType(desiredSide)) {
			wl.Infof("coind-debug: no-op: already in desired state: %s == %s", currentSide, desiredSide)
			// log signal and no-op
			err = ss.insertLatestSignalTradedForStrategy(ctx, wl, chain, signal, strategy)
			if err != nil {
				return err
		...
		wl.Debugf("coind-debug: useFundingCurrency: %t", useFundingCurrency)
	...
	orderPayload = coinroutesapi.ClientOrderCreateRequest{
		...
		UseFundingCurrency: useFundingCurrency, // false for coinM
		// EndOffset:          tradeTTL,
...
func (ss *signalService) calculateCoinValue(
	...
	return (position.Quantity /
		marketPrice /
		float64(strategy.AccountLeverage.ValueOrZero())) +
		balance +
...
func (ss *signalService) calculateCurrentSide(
	...
	if strategy.Margin == entities.CoinD {
		if positionQuantity == 0 {
			...
			wl.Debugf("coind-debug: invalid position side found:%s", position.Side)
			return entities.Neutral, fmt.Errorf("invalid position side found:%s", position.Side)
		}
	...
	if position.Side == string(entities.Buy) || position.Side == string(entities.Long) {
		return entities.Buy, nil
	}
...
func (ss *signalService) calculateTradeAmount(
	...
	if strategy.Margin == entities.CoinD {
		return desiredExposure - currentContractExposure, nil
	}
