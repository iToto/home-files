...
type Config struct {
	ResourcePath string
	BTCPayload   interface{}
	ETHPayload   interface{}
}

type Consumer struct {
	conf            Config
	btcLastResponse []byte
	ethLastResponse []byte
}
...
func NewConsumer(conf Config) (*Consumer, error) {
	...
	if conf.BTCPayload == nil {
		return nil, fmt.Errorf("missing mandatory config: BTCPayload")
	}
	if conf.ETHPayload == nil {
		return nil, fmt.Errorf("missing mandatory config: ETHPayload")
	}
...
func (c *Consumer) GetBTCPrice() (float64, error) {
	r := RealPriceResponse{}
	err := json.Unmarshal(c.btcLastResponse, &r)
	if err != nil {
		return 0.0, err
	}
...
func (c *Consumer) GetETHPrice() (float64, error) {
	r := RealPriceResponse{}
	err := json.Unmarshal(c.ethLastResponse, &r)
	if err != nil {
		return 0.0, err
	}
