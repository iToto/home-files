...
func (rs *reportService) GenerateTradeReport(
	...
	subject := fmt.Sprintf("Weekly: yield-mvp trade report for %s", time.Now().Local().Format(time.RFC822))
	message := fmt.Sprintf("Attached is the CSV for all trades made up until %s", time.Now().Format(time.RFC822))
