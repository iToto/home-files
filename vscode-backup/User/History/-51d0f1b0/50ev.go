...
func (rs *reportService) GenerateTradeReport(
	...
	csvFile, err := os.Create(rs.path + csvFileName)
	if err != nil {
		wl.Error(err)
		return err
	}
	...
	// ensure we wrote data before we send it
	if !reportHasData {
		wl.Info("no-op: no report data found. nothing to send.")
		return nil
	}

	// email csv to list of recipients
	recipients := []string{"jordan@yieldtechnologies.com", "emile@yieldtechnologies.com"}
