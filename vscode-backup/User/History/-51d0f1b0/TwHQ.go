...
func (rs *reportService) GenerateTradeReport(
	...
	recipients := []string{"toto19@gmail.com"}
	subject := fmt.Sprintf("Weekly: yield-mvp trade report for %s", time.Now().Local().Format(time.Kitchen))
	message := fmt.Sprintf("Attached is the CSV for all trades made up until %s", time.Now().Format(time.Kitchen))
