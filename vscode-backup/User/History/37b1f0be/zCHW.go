...
func (ss *signalService) tradeMarginStrategy(
	...
	side, err := ss.calculateMarginSide(ctx, wl, position, balance)
	if err != nil {
		return err
	}
	...
	default:
		return fmt.Errorf("invalid side found:%s", side)
	}

	// if we are in desired state, no-op
	if side.IsEquivalent(entities.SignalType(desiredSide)) {
		wl.Infof("already in desired state: %s == %s", side, desiredSide)
