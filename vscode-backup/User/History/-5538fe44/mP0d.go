...
func (c *Client) GetStrategyBalances(ctx context.Context, wl wlog.Logger) (exchangeclient.StrategyBalance, error) {
	...
	// get balances for each strategy we have a config for
	for _, strategy := range c.conf.Stategies {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to create request: %w for strategy %s", err, strategy.Name)
		}

		var res rawBalanceResponse
		if err := c.httpClient.DoJSON(req, &res); err != nil {
			wl.Debugf("failed to do request: %w for strategy %s", err, strategy.Name)
			return nil, fmt.Errorf("failed to do request: %w", err)
		}

		// log raw response for debugging
		wl.Debugf("raw balance response for strategy %s: %+v", strategy.Name, res)

		if len(res.Data) < 1 {
			// append empty balance for this strategy
			balance := &exchangeclient.Balance{
				BTCAvailableBalance:          "no balance found",
				BTCInitialMarginRequirement:  "no balance found",
				BTCEquityOfCurrency:          "no balance found",
				ETHAvailableBalance:          "no balance found",
				ETHInitialMarginRequirement:  "no balance found",
				ETHEquityOfCurrency:          "no balance found",
				USDTAvailableBalance:         "no balance found",
				USDTInitialMarginRequirement: "no balance found",
				USDTEquityOfCurrency:         "no balance found",
				USDCAvailableBalance:         "no balance found",
				USDCInitialMarginRequirement: "no balance found",
				USDCEquityOfCurrency:         "no balance found",
			}
			strategyBalances[strategy.Name] = balance
			wl.Debugf("balance for strategy %s: %+v", strategy.Name, balance)
			continue
		}
...
func (c *Client) GetStrategyPositions(ctx context.Context, wl wlog.Logger) (exchangeclient.StrategyPosition, error) {
	...
	for _, strategy := range c.conf.Stategies {
		...
		// log raw response for debugging
		wl.Debugf("raw position response for strategy %s: %+v", strategy.Name, res)

		if len(res.Data) < 1 {
			position := &exchangeclient.Position{
				Position:     "no position found",
				NotionalUSD:  "no position found",
				UnrealizedPL: "no position found",
				MarketPrice:  "no position found",
			}
			strategyPositions[strategy.Name] = position
			wl.Debugf("position for strategy %s: %+v", strategy.Name, position)
			continue
		}
...
func (c *Client) generateRequestHeaders(
	...
	// timestamp in format 2020-12-08T09:08:57.715Z
