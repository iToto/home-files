...
func (c *Client) GetStrategyBalances(ctx context.Context, wl wlog.Logger) ([]*exchangeclient.StrategyBalance, error) {
	var strategyBalances []*exchangeclient.StrategyBalance
	...
	for _, strategy := range c.conf.Stategies {
		...
		strategyBalances = append(strategyBalances, &exchangeclient.StrategyBalance{
			Name:    strategy.Name,
			Balance: balance,
		})
...
func (c *Client) GetStrategyPositions(ctx context.Context, wl wlog.Logger) ([]*exchangeclient.StrategyPosition, error) {
	var strategyPositions []*exchangeclient.StrategyPosition
	...
	for _, strategy := range c.conf.Stategies {
		...
		strategyPositions = append(strategyPositions, &exchangeclient.StrategyPosition{
			Name:     strategy.Name,
			Position: position,
		})
	}
