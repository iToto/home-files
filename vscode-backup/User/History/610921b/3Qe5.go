		...
		case <-ticker.C:
			...
			if err != nil {
				...
				if errors.Is(err, signalsvc.ErrSignalClient) {
					st.retryCount += 1
					wl.Error(fmt.Errorf("error when calling signal client, adding backoff to ticker of %d seconds", (st.backoff * st.retryCount)))
					ticker.Reset(time.Duration(st.backoff*st.retryCount) * time.Second)
				}
			} else {
