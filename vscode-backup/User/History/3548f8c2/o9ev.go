...
func checkForSignal(
	...
	if err != nil {
		...
		signal.Signal = SignalType(sig.LastTrade)
	...
	if lastSignal.Signal == SignalType(sig.LastTrade) &&
		lastSignal.TradeTime.Equal(sig.LastTradeTime) {
		wl.Debugf(
			"no-op: %s == %s && %s == %s",
			lastSignal.Signal,
			SignalType(sig.LastTrade),
...
func parseSignalAndCreateTradePayload(
	...
	if sig.Signal == Buy || sig.Signal == Long {
		// Handle Buy/Long
	...
	} else if sig.Signal == Sell || sig.Signal == Short {
		// Handle Sell/Short
		payload := coinroutesapi.ClientOrderCreateRequest{
			OrderType:          coinroutesapi.SmartPost,
			OrderStatus:        coinroutesapi.Open,
