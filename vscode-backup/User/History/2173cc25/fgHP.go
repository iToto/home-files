...
func (ss *signalService) checkForSignal(
	...
	receivedSignal := &entities.Signal{
		ID:       id.String(),
		Chain:    sig.Chain,
	...
	// normalize Buy/Sell
	if receivedSignal.Signal == entities.Long {
		receivedSignal.Signal = entities.Buy
	}

	if receivedSignal.Signal == entities.Short {
		receivedSignal.Signal = entities.Sell
	}
	...
	// log strategy type
	wl.Debugf("strategy type: %s", strategy.Type)

	// override sell signal to neutral if we are using long neutral strategy type
	if strategy.Type == entities.LongNeutral && receivedSignal.Signal == entities.Sell {
		wl.Debug("overriding sell signal to neutral for long neutral strategy")
		sig.LastTradeSignal = signalapi.Neutral
		receivedSignal.Signal = entities.Neutral
	}

	// override buy signal to neutral if we are using short neutral strategy type
	if strategy.Type == entities.ShortNeutral && receivedSignal.Signal == entities.Buy {
		wl.Debug("overriding buy signal to neutral for short neutral strategy")
		sig.LastTradeSignal = signalapi.Neutral
		receivedSignal.Signal = entities.Neutral
	}
	...
	if err != nil {
		...
		if errors.Is(err, ErrDBConnection) {
			...
			// log signal to BQ
			errLog := ss.sl.Log(ctx, wl, delta, sig, receivedSignal.ID, strategy.Name) // FIXME: Should be passing internal entities
			if errLog != nil {
		...
		if errors.Is(err, ErrNoSignalHistory) {
			...
			// log signal to BQ
			logErr := ss.sl.Log(ctx, wl, delta, sig, receivedSignal.ID, strategy.Name) // FIXME: Should be passing internal entities
			if logErr != nil {
				wl.Infof("unable to log signal to bigquery: %w", err)
			}

			// return last signal to be logged
			receivedSignal.Signal = entities.SignalType(sig.LastTradeSignal)
			receivedSignal.TradeTime = sig.LastTradeSignalTime

			return receivedSignal, err
		}
		...
		return receivedSignal, ErrNoSignalHistory
	}
	...
	if lastSignal == nil {
		...
		// log signal to BQ
		err = ss.sl.Log(ctx, wl, delta, sig, receivedSignal.ID, strategy.Name) // FIXME: Should be passing internal entities
		if err != nil {
			wl.Infof("unable to log signal to bigquery: %w", err)
		}
	...
	if lastSignal.Signal.IsEquivalent(receivedSignal.Signal) {
		wl.Infof(
			"no-op: %s == %s && %s == %s",
			lastSignal.Signal,
		...
		receivedSignal.Signal = entities.Null
		receivedSignal.TradeTime = lastSignal.TradeTime
