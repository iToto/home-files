...
func WithServiceRequest(ctx context.Context, l Logger, serviceName string) Logger {
	...
	if signalIP, ok := ctx.Value(acontext.ContextKeySignalIP).(string); ok {
		l = l.WithStr(LogKeySignalIP, signalIP)
	}
...
// WithSignalIP adds the requested signal IP to the logger
func WithSignalIP(l Logger, ip string) Logger {
	return l.WithStr(LogKeySignalIP, ip)
}
