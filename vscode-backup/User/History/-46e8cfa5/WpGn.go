...
func (s *exchangeService) GenereateReport(
	...
	balances, err := s.exchangeClient.GetBalance(ctx, wl)
	if err != nil {
		return nil, err
	}

	positions, err := s.exchangeClient.GetPosition(ctx, wl)
	if err != nil {
		return nil, err
	}

	wl.Debugf("balances: %v", balances)
	wl.Debugf("positions: %v", positions)
