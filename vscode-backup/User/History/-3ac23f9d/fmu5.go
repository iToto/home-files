...
func (ssd *signalSourceDAL) GetSignalSources(
	ctx context.Context,
	wl wlog.Logger,
) ([]*entities.SignalSource, error) {
	var rows []entities.SignalSource
	...
	query := `SELECT 
	id, enabled, type, ip, signal_version, created_at, updated_at, deleted_at 
	FROM "public"."mvp_signal_source"
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("no signals found")
		}
