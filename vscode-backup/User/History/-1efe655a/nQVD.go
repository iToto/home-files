...
func (ss *signalService) GetAndProcessSignal(
	...
	for _, currentStrategy := range signalStrategy.Strategies {
		...
		// check for CoinD strategy
		if currentStrategy.Margin == entities.CoinD {
			wl.Debug("processing CoinD strategy")
			err := ss.tradeCoinDStrategy(
				ctx,
				wl,
				signalStrategy.Chain,
				currentStrategy,
