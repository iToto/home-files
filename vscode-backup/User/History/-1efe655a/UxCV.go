...
func (ss *signalService) GetAndProcessSignal(
	...
	wl1.Debugf("processing signal: %+v", signalStrategy.Signal)
	wl1.Debugf("with strategies: %+v", signalStrategy.Strategies)
	...
	if err != nil {
		wl1.Error(fmt.Errorf("error when calling signal client: %w", err))
	...
	for _, currentStrategy := range signalStrategy.Strategies {
		...
		// check for ConinM strategy
		if currentStrategy.Margin == entities.CoinM {
			wl.Debug("processing CoinM strategy")
			err := ss.tradeCoinMStrategy(
				ctx,
				wl,
				signalStrategy.Chain,
				currentStrategy,
				desiredSide,
				signal,
			)
			if err != nil {
				wl.Error(fmt.Errorf("error while processing coin margin strategy: %w", err))
				continue
			}
			continue
		}

		// check for spot strategy
		if currentStrategy.Margin == entities.Spot {
			wl.Debug("processing spot strategy")
			err = ss.tradeSpotStrategy(
				ctx,
				wl,
				signalStrategy.Chain,
				currentStrategy,
			...
			if err != nil {
				wl.Error(fmt.Errorf("error while processing spot strategy: %w", err))
				continue
			}
			continue
		}
		...
		*********************************/
		...
		if currentStrategy.Margin == entities.USDTM {
			wl.Debug("processing USDTM strategy")
			err := ss.tradeUSDTStrategy(ctx, wl, signalStrategy.Chain, currentStrategy, desiredSide, signal)
			if err != nil {
		...
		/**************************************************
		* non USDT or CoinM strategies (To be deprecated) *
		**************************************************/

		wl.Info("unhandled strategy type.")
		// If we don't see this log line, then we can deprecate all lines under this.
