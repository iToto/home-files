...
func (od *orderDAL) GetOrderReportForStrategy(
	ctx context.Context,
	wl wlog.Logger,
	strat string,
) ([]*entities.OrderReportRecord, error) {
	...
	query := `select o.strategy, o.coin as coin, o.signal_id as signal_id, o.created_at as created_at, o.side as direction, o.signal as signal, o.avg_price, o.executed_qty
	from mvp_order o
	join mvp_strategy st on st."name" = o.strategy
	where st.enabled = true AND strategy = $1 AND o.status = 'closed' AND coin IS NOT NULL
	order by strategy, o.created_at desc`
	err := od.db.Select(&rows, query, strat)
	if err != nil {
	...
	for _, row := range rows {
		record := entities.OrderReportRecord{
