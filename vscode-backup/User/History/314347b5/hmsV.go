...
func (c *Client) CreateClientOrders(
	...
	// check if quantity has not been set
	if order.Quantity == "" {
		// return nil, fmt.Errorf(
		// 	"quantity has not been set, aborting request with order: %+v",
		// 	order,
		// )
	}
...
func (c *Client) GetBalances(ctx context.Context) (*[]CurrencyBalanceResponse, error) {
	url := fmt.Sprintf("%s/api/currency_balances/", c.conf.URL)
	req, err := c.buildRequest(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, err
	}
