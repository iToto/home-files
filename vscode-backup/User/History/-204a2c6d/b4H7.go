...
func (ss *signalService) queryLastSignal(
	...
	var lastSignal signalLog

	query := "SELECT id, chain, signal, trade_time, created_at, updated_at, deleted_at FROM mvp_signal_log WHERE signal = $1 AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 1"
