...
func (ss *signalService) queryLastSignal(
	...
	query := "SELECT id, ip, chain, signal, trade_time, created_at, updated_at, deleted_at FROM logger_signal_log WHERE signal = $1 AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 1"
	err := ss.db.Get(&lastSignal, query, signal)
	if err != nil {
...
func (ss *signalService) insertLatestSignalTradedForStrategy(
	...
	query := `INSERT INTO logger_signal_log (chain, signal, trade_time, created_at) 
