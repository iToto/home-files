...
import (
	...
	"net/http"
	"os"
	"social-links-api/cmd/social-links-api/internal/handler"
	"social-links-api/internal/service/hellosvc"
	"social-links-api/internal/wlog"
	"social-links-api/pkg/utils"
...
func main() {
	...
	socialService, err := socialsvc.New(db, baseURL)
	if err != nil {
		return nil, log.Fatal("unable to init social service: %w", err)
	}
	...
	router.HandleFunc(
		"social-links",
		handler.CreateSocialURL(wl, socialService)).
