...
func WithServiceRequest(ctx context.Context, l Logger, serviceName string) Logger {
	...
	if chain, ok := ctx.Value(ycontext.ContextKeyChain).(string); ok {
		l = l.WithStr(LogKeyChain, chain)
	}
...
// WithChain adds the requested chain to the logger (BTC/ETH)
func WithChain(l Logger, chain string) Logger {
	return l.WithStr(LogKeyChain, chain)
}
