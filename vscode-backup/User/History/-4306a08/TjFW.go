...
func (ss *signalService) GetAndProcessSignal(
	...
	for _, signal := range signals {
		...
		if signal.SignalVersion == entities.V1 {
			...
			// check for delta
			if prevSignal != nil {
				if prevSignal.Signal.IsEquivalent(entities.SignalType(signalResp.Signal)) &&
			...
			s := signalHistory{
				...
				Signal:    entities.SignalType(signalLog.Signal),
			...
			// check for delta
			if prevSignal != nil {
				if prevSignal.Signal.IsEquivalent(entities.SignalType(signalResp.StrategyState)) &&
			...
			s := signalHistory{
				ID:        signalLog.ID,
				Chain:     signalLog.Chain,
				Signal:    entities.SignalType(signalLog.StrategyState),
