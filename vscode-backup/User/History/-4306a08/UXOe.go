...
func (ss *signalService) GetAndProcessSignal(
	...
	for _, signal := range signals {
		...
		if signal.SignalVersion == entities.V1 {
			...
			// check for delta
			if prevSignal != nil {
				if prevSignal.Signal.IsEquivalent(entities.SignalType(strings.ToLower(signalLog.LastTrade))) &&
			...
			// check for delta
			if prevSignal != nil {
				if prevSignal.Signal.IsEquivalent(entities.SignalType(strings.ToLower(signalLog.LastTradeSignal))) &&
					prevSignal.TradeTime.Equal(signalLog.LastTradeSignalTime) {
					wl.Info("no signal change, skipping")
					continue
				} else {
					wl.Debugf("prev: %s %s", prevSignal.Signal, prevSignal.TradeTime)
					wl.Debugf("log: %s %s", signalLog.LastTradeSignal, signalLog.LastTradeSignalTime)
					wl.Debugf("equivalent: %b", prevSignal.Signal.IsEquivalent(entities.SignalType(signalLog.LastTradeSignal)))
					wl.Debugf("equal: %b", prevSignal.TradeTime.Equal(signalLog.LastTradeSignalTime))
				}
			}
			...
			s := signalHistory{
				...
				Chain:     signalLog.Chain,
				IP:        signalLog.IP,
				Signal:    entities.SignalType(signalLog.LastTradeSignal),
