...
func (ss *signalService) GetAndProcessSignal(
	...
	for _, signal := range signals {
		wl.Debugf(
			"========= db connections: open %d use %d idle %d =========",
			ss.db.Stats().OpenConnections,
			ss.db.Stats().InUse,
			ss.db.Stats().Idle,
		)
		...
		if signal.SignalVersion == entities.V1 {
			...
			if err != nil {
				wl.Error(fmt.Errorf("error getting signal"))
			...
			if err != nil {
				wl.Infof("unable to log signal with error: %w", err)
				continue
			}
		} else {
			...
			err = ss.insertLatestSignalTradedForStrategy(ctx, wl, s)
			if err != nil {
				wl.Infof("unable to insert signal into log table: %w", err)
				return err
			}
			...
			if err != nil {
				wl.Infof("unable to log signal v2 with error: %w", err)
				continue
			}
		}
