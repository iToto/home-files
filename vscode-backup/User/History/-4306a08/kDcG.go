...
func (ss *signalService) GetAndProcessSignal(
	...
	for _, signal := range signals {
		...
		if signal.SignalVersion == entities.V1 {
			...
			// check for delta
			if prevSignal != nil {
				if prevSignal.Signal.IsEquivalent(entities.SignalType(strings.ToLower(signalResp.LastTrade))) &&
			...
			// check for delta
			if prevSignal != nil {
				if prevSignal.Signal.IsEquivalent(entities.SignalType(signalLog.LastTradeSignal)) &&
